        ...
        <Row :gutter="10">
            ...
            <Card>
                ...
                <Select v-model="taskId"
                    ...
                    <Option v-for="item in tasks" :value="item.id" :key="item.id" v-if="tasks.length !== 0">{{ item.task }}</Option>
                </Select>
                <Row>
                    ...
                    <Form :model="formData" ref="formData" v-if="staffs.length !== 0">
                        ...
                        <Row v-for="(staff,index) in staffs" class="staff" :class="active[index] ? 'active':''">
                            <div @click="clickStaff(index)">
                                <Col span="6">{{ staff.staff}}（{{ staff.position}}）</Col>
                                <Col span="6">
                                ...
                                <Col span="6">
                                  ( 绩效权重{{ staff.pivot.mark ? (staff.pivot.mark / finMark[staff.id] *100).toFixed()+'%': '0' }} )
                                </Col>
                            </div>
                        </Row>
        ...
        data () {
            return {
                ...
                markRange: [
                    ...
                    {
                        'min':90,'max':95,'appraise':'优良','introduce':'B','active':''
                    },
                    {
                        'min':80,'max':89,'appraise':'合格','introduce':'C','active':''
                    },
                    {
                        'min':69,'max':79,'appraise':'不足','introduce':'D','active':''
                    },
                    {
                        'min':1,'max':59,'appraise':'不合格','introduce':'E','active':'activeError'
                    }
                ],
                loading: false,
                active: [],
        ...
        methods: {
            ...
            handleSubmit (name) {
               ...
               if(this.formData.staffId.length !== 0){
                   ...
                   this.JAjax.postJson('note/month/task',this.formData,res=>{
            ...
            // 获取项目
            getTasks () {
                this.JAjax.postJson('task/all/list', {date:this.toMonth(this.date)}, res => {
                    this.tasks = res.data || [];
