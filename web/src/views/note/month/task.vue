...
<script>
    export default {
        ...
        methods: {
            alertMessage (){
                this.isSubmit = true
                this.JAjax.postJson('note/is_depart/task',{
                    date:this.date
                },res=>{
                   if(res.data && this.attribute){
                           this.$Modal.warning({
                               title: '提示',
                               content: "尚有项目未录入，无法先录入部门中心项目"
                           });
                       this.isSubmit = false
                   }
                })
                // if(this.attribute && this.tasks.length > 1){
                //     this.$Modal.warning({
                //         title: '提示',
                //         content: "尚有项目未录入，无法先录入部门中心项目"
                //     });
                    ...
                    if (t.id === this.taskId) {
                        ...
                        t.staffs.forEach((e) => {
                            if (e.leave_at === null) {
                                ...
                                // this.formData.isMark[e.id] = this.attribute ? '1' : '0'
                                this.formData.isMark[e.id] = '0'
                                this.formData.mark[e.id] = 1
                                this.getMarkRange(e.id,0)
                                this.staffs.push(e);
